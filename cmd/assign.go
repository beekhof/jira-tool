...
func runAssign(cmd *cobra.Command, args []string) error {
	...
	if unassignFlag {
		return unassignMultipleTickets(client, cfg, allFlag, stateFlag)
	}
	return assignMultipleTickets(client, cfg, allFlag, stateFlag)
}
...
func assignMultipleTickets(client jira.JiraClient, cfg *config.Config, allFlag bool, stateFlag string) error {
...
func init() {
	assignCmd.Flags().BoolVar(&unassignFlag, "unassign", false, "Unassign tickets instead of assigning them")
	assignCmd.Flags().BoolVar(&allFlag, "all", false, "Show all tickets regardless of state (default: only Backlog)")
